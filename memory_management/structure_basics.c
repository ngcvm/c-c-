...
int main (int argc, char* argv[]) {
    // Declare and initialize struct Grade on the stack
    const Grade student_grade = { 'A', 96 };

    // Read value from a struct's member
    ...
    printf("Size of 'student_grade.letter' is (byte):\t\t%zu\n", letter_byte_size);
    printf("Size of 'student_grade.rank' is (byte):\t\t%zu\n", rank_byte_size);

    const size_t student_grade_byte_size = sizeof(student_grade);
    printf("Size of 'student_grade' is (byte):\t\t%zu\n", student_grade_byte_size);
    
    // Calculate necessary padding via sizes of structure members
    const size_t total_padding_in_grade_struct = student_grade_byte_size - (letter_byte_size + rank_byte_size);
    printf("Total padding of Grade structure is (byte):\t\t%zu\n", total_padding_in_grade_struct);
    
    // Reveal memory address position of struct members
    ...
    printf("Letter byte offset is (byte):\t\t\t\t%zu\n", letter_offset);
    printf("Rank byte offset is (byte):\t\t\t\t%zu\n", rank_offset);
    printf("Total padding of Grade structure is (byte):\t\t\t\t%zu\n", padding);
