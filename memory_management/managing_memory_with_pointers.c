/**
 * This file simulates a possible real world scenario. The point of this C program is Wired Brain Coffee has hired you to help fix their code so that they manage
 * memory more efficiently with less bugs. In the code below, there is a memory leak being created in the form of a dangling pointer.
 * 
 * Your mission is to identify and fix the memory leak that is occurring due to one of the 'Wired Brain Coffee' API functions below.
 * 
 * NOTE: In a real-world C library/program, there would be a header file associated with this C file in order to better
 * define a usable, public API/interface and to separate declarations from instantiations. Function and structure definitions are
 * only included in this C file in order to condense the number of files needed in order to follow along with this course so as to reducing context switching.
 **/
...
void cleanup_machine(CoffeeMachine *machine) {
    // Because we have allocated machine metrics, we have to free up those allocated blocks
    ...
    // Then we have to free the machine->metrics pointer
    free(machine->metrics);

    // Lastly, we can free the machine pointer
    free(machine);
}
